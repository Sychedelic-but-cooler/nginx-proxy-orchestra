 ...
 * Uses nginx include directive to keep configs clean
...
function renderServerLevelModules(modules, proxyHost) {
  const { getModuleIncludePath } = require('./module-file-manager');
  const serverModules = modules.filter(m => m.level === 'server');
  let config = '';

  for (const module of serverModules) {
    const includePath = getModuleIncludePath(module.name);
...
function generateServerBlock(proxyHost, modules = [], db = null) {
  ...
  config += `    # Routes requests to the backend application\n`;
  config += `    # ───────────────────────────────────────────────────────────────\n`;
  config += `    location / {\n`;
...
function generateStreamBlock(proxyHost) {
  ...
  let config = `# ═══════════════════════════════════════════════════════════════════\n`;
  config += `# Stream Proxy: ${proxyHost.name} (${protocol.toUpperCase()})\n`;
  config += `# Generated by Nginx Proxy Orchestra\n`;
  config += `# ═══════════════════════════════════════════════════════════════════\n\n`;
...
function generate404Block(proxyHost) {
  ...
  let config = `# ═══════════════════════════════════════════════════════════════════\n`;
  config += `# 404 Host: ${proxyHost.name}\n`;
  config += `# Generated by Nginx Proxy Orchestra\n`;
